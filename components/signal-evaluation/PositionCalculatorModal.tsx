...
export function PositionCalculatorModal({
  ...
  const availableCash = cashInput.trim() ? parseFloat(cashInput.replace(/\D/g, '')) || capital : capital
  ...
  return (
    ...
    >
      <div className="space-y-5 pt-2 text-xs text-[var(--rasi-text)]">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              value={capitalInput ? Number(capitalInput.replace(/\D/g, '')).toLocaleString('id-ID') : ''}
              onChange={(e) => setCapitalInput(e.target.value)}
              className="mt-1.5 w-full rounded-lg border border-[var(--rasi-border)] bg-[var(--rasi-surface)] px-3 py-2 text-sm font-mono text-[var(--rasi-text)] outline-none focus:border-[var(--rasi-primary)]"
            />
          </div>
        ...
        <div>
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono font-bold text-[var(--rasi-primary)]">
              {riskPct.toFixed(1)}% ({capital > 0 ? `Rp ${(capital * (riskPct / 100)).toLocaleString('id-ID')}` : 'Rp 0'})
            </span>
          </div>
        ...
        <div className="rounded-xl border border-[var(--rasi-border)] bg-[var(--rasi-muted-bg)]/40 p-4 space-y-3">
        ...
        <div className="rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-card)] p-3 text-[11px] text-[var(--rasi-muted)] space-y-1">
          <p>
            • <strong>Biaya per saham:</strong> Rp {costBasis.toLocaleString('id-ID')} (sudah termasuk fee beli 0,15%).
          </p>
          <p>
            • <strong>Risiko bersih per saham (R):</strong> Rp {netRiskPerShare.toLocaleString('id-ID')} (memperhitungkan asumsi slippage stop 1 tick dan fee jual 0,25%).
          </p>
          <p>
            • <strong>Aturan lot ganjil:</strong> Jika total lot ganjil, porsi TP1 dialokasikan <code>floor(lots/2)</code>. Jika hanya 1 lot, 100% diposisikan untuk ditutup pada TP1.
          </p>
        </div>
