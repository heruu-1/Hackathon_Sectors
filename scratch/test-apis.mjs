...
await testApi(
  ...
  'https://api.sectors.app/v2/company/report/BBCA/?sections=valuation',
)
...
await testApi(
  '2a. Arus Broker (Summary)',
  `https://api.sectors.app/v2/broker-summary/BBCA/?start=${tenDaysAgo}&end=${today}`,
)
...
await testApi(
  '4. Transaksi Orang Dalam (Filings)',
  'https://api.sectors.app/v2/filings/?symbol=BBCA&limit=5',
)
...
const d30 = await testApi(
  '5a. Daily 30 days',
  `https://api.sectors.app/v2/daily/BBCA/?start=${thirtyDaysAgo}&end=${today}`,
)
const d60 = await testApi(
  '5b. Daily 60 days',
  `https://api.sectors.app/v2/daily/BBCA/?start=${sixtyDaysAgo}&end=${today}`,
)
const d90 = await testApi(
  '5c. Daily 90 days',
...
if (d30.data && Array.isArray(d30.data)) {
  ...
  console.log(
    `--> Is count >= 21? ${d30.data.length >= 21 ? 'YES' : 'NO (KARENA ITU SMA-20 JADI "-")'}`,
  )
}
