...
try {
  const rows =
    await sql`SELECT cache_key, expires_at, created_at, (data->>'data')::jsonb as inner_data FROM api_cache WHERE cache_key LIKE '%daily%'`
  ...
  const snaps =
    await sql`SELECT ticker, created_at, (payload->'envelopes'->'daily'->>'data')::jsonb as daily_rows FROM analysis_snapshots ORDER BY created_at DESC LIMIT 5`
